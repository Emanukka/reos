...
def linspace_wsat(eos,T,ydg,pi=1,pf=600,N=100):
  ...
  for (i,P_BAR) in enumerate(PRESS_BAR):
    ...
    try:
      ...
      result,state=tpd_root_given_tp(T,P_PA,ydg)
...
for j,state in enumerate(stateResult[i]):
    ...
    # idx 1 - agua positivo 
    # idx 2 - co2 positivo
    AGUA_NEGATIVO[j],AGUA_POSITIVO[j],CO2_POSITIVO[j]=state.non_bonded_sites()
    ...
    print(
       (1-agua_negativo)*2*yw,
       (1-agua_positivo)*2*yw + co2_positivo*yco2
       )
    # AGUA_NEGATIVO[j]*=yw
    # AGUA_POSITIVO[j]*=yw
    # CO2_POSITIVO[j]*=yco2
    # print(state.pressure()/1e5)

    # AGUA_POSITIVO[0]=state.non_bonded_sites()[0]

    # xasc[i]=state.non_bonded_sites()[0]

#%%

plt.scatter(pResultBAR[i],AGUA_NEGATIVO,label="H2O -")
plt.scatter(pResultBAR[i],AGUA_POSITIVO,label="H2O +")
plt.scatter(pResultBAR[i],CO2_POSITIVO,label="CO2 +")
plt.xlabel("P/BAR")
...
#%%Monomero
AGUA_MONOMERO=AGUA_NEGATIVO*AGUA_NEGATIVO*AGUA_POSITIVO*AGUA_POSITIVO

plt.scatter(pResultBAR[i],AGUA_MONOMERO,label="H2O")
plt.scatter(pResultBAR[i],CO2_POSITIVO,label="CO2")
plt.xlabel("P/BAR")
plt.ylabel("Monomer mole fraction")
plt.legend()
